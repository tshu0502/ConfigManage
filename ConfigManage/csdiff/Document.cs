...
namespace csdiff
{
	...
	{
		public string[] fnames = new string [2]; //î‰ärÉtÉ@ÉCÉãñºÅH
		public ListAnchor[] lines;               //ÉäÉXÉgÇÃíËã`
		//! public CFileStatus	fstat[2];
		public ListAnchor secComposit = new ListAnchor();  //

		public Document()
		{
			lines = new ListAnchor[2];                  //çsåQÇÃíËã`
			lines[0] = new ListAnchor();                //î‰ärå≥ÇÃçsåQ
			lines[1] = new ListAnchor();                //î‰ärêÊÇÃçsåQ
		}

        //î‰ärå≥êÊÇ«ÇøÇÁÇ‡LOADÇ≥ÇÍÇΩÇ©ÅB
        public bool IsLoadedAll()
		{
			return IsLoaded(DocOf.LEFT) && IsLoaded(DocOf.RIGHT);
		}

        //î‰ärå≥orêÊÇÃçsÇ™LOADÇ≥ÇÍÇΩÇ©ÅB
        public bool IsLoaded(DocOf nLeftorRight)
		{
			return !lines[(int)nLeftorRight].IsEmpty();
		}

        //fnameÇ≈é¶Ç≥ÇÍÇÈÉtÉ@ÉCÉãÇÃLOAD
		...
		/// <summary>
        /// linesLeftÇ∆linesRightÇî‰ärÇ∑ÇÈ
        /// </summary>
        /// <param name="secsComposite">î‰äråãâ </param>
        /// <param name="linesLeft">î‰ärå≥</param>
        /// <param name="linesRight">î‰ärêÊ</param>
        /// <param name="isIgnoreBlanks">èåè</param>
