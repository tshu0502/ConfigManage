...
namespace csdiff
{
	...
	{
		public Line first;			/* first line in section（sectionの最初の行）*/
		public Line last;           /* last line in section （sectionの最後の行）*/
		...
		public uint leftbase;		/* nr in original left list of first line（左のリストの最初の行）*/
		public uint rightbase;      /* nr in original right list of first line（右のリストの最初の行）*/
		...
		* Returns a handle to a list of sections */
         ...
         *複合リストにセクションを追加します。 make_compositesから呼び出されます
         *、セクションをコピー複合リストに追加し、状態を設定するには、
         * leftbaseとrightbase。状態があり得ることに注意してくださいSTATE.SAME
         * 左のゼロ区間で。 STATE.SAMEで呼び出すことはできませんし、
         * nullの右側のセクション！ */
